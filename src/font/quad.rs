...
pub fn quad(p0: vec2, p1: vec2, p2: vec2, mut line_to: impl FnMut(vec2)) {
	...
	use array::IntoIterator;
	kurbo::flatten([MoveTo(p0),QuadTo(p1,p2)].into_iter(), 1./4., |e| if let LineTo(kurbo::Point{x,y}) = e { line_to(vec2{x: x as f32, y: y as f32}); } /*Ignore first MoveTo*/)
}
...
impl Subdivision {
	pub(crate) fn determine_subdiv_t(&self, t: f32) -> f32 {
...
impl Quad {
    ...
    pub(crate) fn estimate_subdiv(&self, tolerance_sqrt: f32) -> Subdivision {
