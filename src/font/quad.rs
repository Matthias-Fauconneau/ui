...
pub fn quad(p0: vec2, p1: vec2, p2: vec2, mut line_to: impl FnMut(vec2)) {
	...
	kurbo::flatten(std::array::IntoIter::new([MoveTo(p0),QuadTo(p1,p2)]), 1./4., |e| if let LineTo(kurbo::Point{x,y}) = e { line_to(vec2{x: x as f32, y: y as f32}); } /*Ignore first MoveTo*/)
}
