...
use {core::{error::{throws, Error, Result}, num::Zero}, ::xy::size, image::{Image, sRGB, bgra8}, crate::{widget::{self, Target}, font::Rasterize}};
		...
		let buffer = {
			...
			for &Rect{min: top_left, max: bottom_right} in &self.graphic.rects {
				...
				if top_left < target.size {
					...
					target.slice_mut(top_left, core::vector::component_wise_min(bottom_right, target.size)-top_left).set(|_| 1.);
				}
			}
			for &Glyph{top_left, id} in &self.graphic.glyphs {
				...
				if offset < target.size {
					...
					let size = core::vector::component_wise_min(coverage.size, target.size-offset);
