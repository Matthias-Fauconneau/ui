...
impl ui::Widget for Glyph<'_> {
	...
	fn paint(&mut self, target: &mut ui::Target, size: ui::size, _: ui::int2) -> Result {
		let glyph = ui::font::Rasterize::rasterize(self.face, num::Ratio{num:size.y-1, div: self.bbox.size().y}, self.id, self.bbox);
		target.set_map(&glyph, |_,&v| image::bgr::from(image::PQ10(1.-v)).into());
		Ok(())
	}
}
fn main() -> Result {
	...
	ui::run("glyph", &mut Glyph{face, id, bbox})
}
